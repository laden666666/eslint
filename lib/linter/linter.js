...
function runRules(sourceCode, configuredRules, ruleMapper, parserOptions, parserName, settings, filename, disableFixes, cwd) {
    ...
    // 真正的执行各个规则。其实就是创建各个规则实例，然后根据他们监听的事件，绑定
    Object.keys(configuredRules).forEach(ruleId => {
        ...
        // 规则这里用事件绑定的方式，注册自己感兴趣的事件（感觉事件名就是 AST 节点的类型名，难到和 traverse 一样）
    ...
    // 果然是这样。利用线性的列表，比树型结构强在哪？？？减少循环时压栈退栈的成本？？？
...
class Linter {
    ...
    verifyAndFix(text, config, options) {
         ...
         */
        /**
         * 这里处理的还是很妙的，考虑一个问题（这里考虑带 fix 的情况）：
         * 是一次校验一个规则，然后立刻改一个规则，还是把所有规则校验完，最后一起改？
         * 方式1：如果 check 一个 fix 一个，就会导致每个规则都要重新 parse、traverse。而且 fix 后可能可能之前面的规则失效，需要重新 check 以前的规则。
         * 方式2：如果 check 所有，再一起 fix，会导致： 1.几个规则改一个地方，会冲突，导致改乱了 2.几个规则本身就是冲突的，死循环？？？
         * 
         * eslint 使用方式2，因此需要考虑解决 方式2 引起的两个问题：
